...
namespace 복사해보기
{
    public partial class Form7 : MetroFramework.Forms.MetroForm
    {
        string strConn = "Data Source=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT=1521)))" +
               "(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=xe)));User Id=hr;Password=kgi;";
        // 오라클 연결
        ...
        private void chPie_Click(object sender, EventArgs e)
        {
            ...
            while (rdr1000.Read())
            {
                ...
                int n_1 = int.Parse(pork_num) * 100000;
                int n_2 = int.Parse(cow_num) * 200000;
                int n_3 = int.Parse(sheep_num) * 200000;
                MessageBox.Show($"{month[0]}의 순수익은 \n 돼지고기 : {n_1}원 \n 소고기 : {n_2}원 \n 양고기 : {n_3}원", "매출액", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
